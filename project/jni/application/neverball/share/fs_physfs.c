...
fs_file fs_open(const char *path, const char *mode)
{
    ...
    if ((fh = malloc(sizeof (*fh))))
    {
        switch (mode[0])
        {
        case 'r': ; // GJT: Extra ; due to GCC bug: http://gcc.gnu.org/bugzilla/show_bug.cgi?id=37231
            #ifdef ANDROID
            static char zipPath[MAXSTR];
            SAFECPY(zipPath, ANDROID_DATA); // GJT: Android APK game contents are in this folder
            SAFECAT(zipPath, path);

            fh->handle = PHYSFS_openRead(zipPath);   
            #else
